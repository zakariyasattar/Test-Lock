...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"assets/js/teacher.js":[function(require,module,exports) {
...
var alphabet = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"; // HAS LOADED

var examDataHasLoaded = false;
var userDataHasLoaded = false; //when page loads, populateDashboard()
...
var profileImg = JSON.parse(localStorage.getItem("userInfo"))[2]; //initialize aspects of page with JS data
...
function displayExamData(examName) {
  if (!examDataHasLoaded) {
        ...
        for (var initData in obj[prop]) {
          if (obj[prop][initData].examCode != undefined && Object.keys(obj[prop]).length > 1) {
            ...
            for (var response in obj[prop].responses) {
              ...
              tr.onclick = function () {
                loadStudentExamData(this.className, examCode);
              };
    ...
    var standardDev = document.createElement('li'); // calc SD

    mean = standardDeviation.reduce(function (a, b) {
      return a + b;
    }) / standardDeviation.length;
    sd = Math.sqrt(standardDeviation.map(function (x) {
...
function loadStudentExamData(name, code) {
  ...
  document.body.style.backgroundImage = "none";
  var answer = "";
  var finalId; //linear-gradient(135deg, rgb(245, 247, 250) 0%, rgb(195, 207, 226) 100%);
  ...
  firebase.database().ref("Teachers/" + userName + "/Classes/" + localStorage.getItem("className") + "/Exams/" + code).once('value', function (snapshot) {
    ...
    for (var i = 0; i < Object.keys(data.answers).length; i++) {
      ...
      createGradedQuestion( // PARAM: studAnswer
      data.answers[i].split(";")[1], // PARAM: corrAnswer
      corrAnswer, // PARAM: numAnswerChoices
      Object.keys(snapshot.val()[Object.keys(snapshot.val())[0]].questions[i].choices).length, // PARAM: numPoints
      snapshot.val()[Object.keys(snapshot.val())[0]].questions[i].points, // PARAM: questions
      snapshot.val()[Object.keys(snapshot.val())[0]].questions[i], // PARAM: studName
      name, // PARAM: examCurrentCode
      code, // PARAM: examTotalPoints
...
function createGradedQuestion(studAnswer, corrAnswer, numAnswerChoices, numAnswerPoints, questions, studName, examCurrentCode, examTotalPoints) {
  ...
  points.appendChild(numPoints);
  points.appendChild(finishingSpan);
    ...
    firebase.database().ref("Teachers/" + userName + "/Classes/" + localStorage.getItem("className") + "/Exams/" + examCurrentCode + "/responses/" + studName).once('value', function (snapshot) {
      snapshot.forEach(function (childSnapshot) {
        ...
        if (answers[2].split(";")[2] != undefined) {
          setTimeout(function () {
            document.getElementsByClassName('pa')[parseInt(num.innerHTML.substring(0, num.innerHTML.indexOf("."))) - 1].innerHTML = " points)  -  points awarded: " + answers[2].split(";")[2];
          }, 100);
    ...
    numAcceptedPoints.onkeyup = function () {
      ...
      firebase.database().ref("Teachers/" + userName + "/Classes/" + localStorage.getItem("className") + "/Exams/" + examCurrentCode + "/responses/" + studName).once('value', function (snapshot) {
        snapshot.forEach(function (childSnapshot) {
          ...
          firebase.database().ref("Teachers/" + userName + "/Classes/" + localStorage.getItem("className") + "/Exams/" + examCurrentCode + "/responses/" + studName + "/" + childSnapshot.key).update({
            'answers': answers
          });
...
function sort(func) {
  ...
  if (func == " Original") {
    ...
    for (var i = 0; i < examData.length; i++) {
      ...
      tr.onclick = function () {
        loadStudentExamData(examData[iTracker].split(":")[0], localStorage.getItem("populatedExamCode"));
      };
    ...
    if (func == " Sort By Alphabet (A-Z)") {
      ...
      for (var i = 0; i < testExamData.length; i++) {
        ...
        tr.onclick = function () {
          loadStudentExamData(testExamData[i].split(":")[0], localStorage.getItem("populatedExamCode"));
        };
      ...
      for (var i = scores.length - 1; i >= 0; i--) {
        ...
        tr.onclick = function () {
          loadStudentExamData(scores[i].split(":")[0], localStorage.getItem("populatedExamCode"));
        };
      ...
      for (var i = times.length - 1; i >= 0; i--) {
        ...
        tr.onclick = function () {
          loadStudentExamData(times[i].split(":")[0], localStorage.getItem("populatedExamCode"));
        };
...
function createExamBox(name, classAvg, ref, code) {
  var gradients = ["linear-gradient( 135deg, #FEB692 10%, #EA5455 100%)", "linear-gradient( 135deg, #72EDF2 10%, #5151E5 100%)", "linear-gradient( 135deg, #ABDCFF 10%, #0396FF 100%)"];
  ...
  option_vertical.onclick = function () {
    ...
    ul.appendChild(document.createElement('hr'));
    ul.appendChild(deleteTest);
    ul.appendChild(document.createElement('hr'));
    ul.appendChild(displayInfo);
...
function createCurrentTakingBox(name) {
  ...
  var gradients = ["linear-gradient(135deg, #667eea 0%, #764ba2 100%)", "linear-gradient(to top, #a3bded 0%, #6991c7 100%)", "linear-gradient(60deg, #29323c 0%, #485563 100%)", "linear-gradient(-60deg, #ff5858 0%, #f09819 100%)", "linear-gradient(to top, #4481eb 0%, #04befe 100%)", "linear-gradient(to right, #0acffe 0%, #495aff 100%)", "linear-gradient(-225deg, #A445B2 0%, #D41872 52%, #FF0066 100%)"];
...
function createMatching() {
  ...
  matching.appendChild(questions);
  matching.appendChild(results);
...
function createQuestion(loading, numAnswerChoices) {
  ...
  mc.value = "mc";
  mc.innerHTML = "Multiple Choice";
  var fr = document.createElement('option');
  fr.value = "fr";
  fr.innerHTML = "Free Response";
  var tf = document.createElement('option');
  tf.value = "tf";
  tf.innerHTML = "True | False"; // var matching = document.createElement('option'); matching.value = "matching"; matching.innerHTML = "Matching"

  question_type.appendChild(mc);
  question_type.appendChild(fr);
  question_type.appendChild(tf); //question_type.appendChild(matching);
  ...
  if (!loading) {
    window.scroll({
      top: question.getBoundingClientRect().top + window.scrollY,
      behavior: 'smooth'
    });
    ...
    ref.once('value', function (snapshot) {
      snapshot.forEach(function (childSnapshot) {
        if (childSnapshot.key == userName) {
          ...
          for (var key in arr) {
            var x = 0; // skip loop if the property is from prototype
...
},{}]},{},["../.nvm/versions/node/v13.0.1/lib/node_modules/parcel-bundler/src/builtins/hmr-runtime.js","assets/js/teacher.js"], null)
//# sourceMappingURL=/teacher.d00ec059.js.map